...
const ServiciosData: React.FC = () => {
    ...
    const [servicios, setServicios] = useState<Categoria[]>(serviciosData);

    const [nuevoServicio, setNuevoServicio] = useState<Subservicio>({
        id: '',
        titulo: '',
        precio: '',
        tiempo: '',
    ...
    const handleAgregarNuevoServicio = (categoriaId: string) => {
        setServicios((prevServicios) => {
            return prevServicios.map((categoria) => {
                if (categoria.id === categoriaId) {
                    return {
                        ...categoria,
                        subservicios: [...categoria.subservicios, { ...nuevoServicio, id: `servicio${Date.now()}` }],
                    };
                }
                return categoria;
            });
    ...
    const handleInputChange = (campo: string, valor: string) => {
        setNuevoServicio((prevNuevoServicio) => ({
            ...prevNuevoServicio,
            [campo]: valor,
        }));
    };
    ...
    return (
        <div>
            <h2 style={{fontSize:'70px'}}>Edición de Servicios</h2>
            {servicios.map((categoria) => (
                <div key={categoria.id}>
                    <h3 style={{fontSize:'30px'}}>{categoria.titulo}</h3>
                    <div className={styles.serviciosContainer}>
                        {categoria.subservicios.map((servicio) => (
                            <div key={servicio.id} className={styles.servicioCard}>
                                <h4>{servicio.titulo}</h4>
                                <p>Explicación: {servicio.explicacion}</p>
                                <p>Precio: {servicio.precio}</p>
                                <p>Tiempo: {servicio.tiempo}</p>
                            </div>
                        ))}
                        <div className={styles.servicioCard}>
                            <form onSubmit={(e) => handleSubmit(categoria.id, e)} className={styles.servicioForm}>
                                <input
                                    type="text"
                                    placeholder="Título"
                                ...
                                <input
                                    ...
                                    value={nuevoServicio.explicacion}
